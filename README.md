<div align="center">

# Hi 👋, I'm Harsh Katiyar

### SOC Analyst | Threat Detection | SIEM | Incident Response | Cybersecurity Enthusiast

<img src="https://visitor-badge.laobi.icu/badge?page_id=HARSH-KATIYAR.HARSH-KATIYAR&left_text=Profile%20Views" />

</div>

---

## 🚀 About Me

* 🔍 Focused on SOC Operations, Threat Detection & Blue Teaming
* 🛡️ Learning SIEM, Splunk, Microsoft Sentinel & Incident Response
* 📊 Interested in Log Analysis, Threat Hunting & Security Monitoring
* 🧠 Building hands-on cybersecurity labs and automation tools
* 🎯 Goal: Become a professional SOC Analyst / Cybersecurity Analyst

---

## 🛠️ Skills & Technologies

<div align="left">

<img src="https://skillicons.dev/icons?i=py" height="55" alt="Python" />
<img width="10" />

<img src="https://skillicons.dev/icons?i=linux" height="55" alt="Linux" />
<img width="10" />

<img src="https://skillicons.dev/icons?i=bash" height="55" alt="Bash" />
<img width="10" />

<img src="https://skillicons.dev/icons?i=mysql" height="55" alt="MySQL" />
<img width="10" />

<img src="https://skillicons.dev/icons?i=js" height="55" alt="JavaScript" />
<img width="10" />

<img src="https://skillicons.dev/icons?i=html" height="55" alt="HTML" />
<img width="10" />

<img src="https://skillicons.dev/icons?i=css" height="55" alt="CSS" />
<img width="10" />

<img src="https://skillicons.dev/icons?i=php" height="55" alt="PHP" />
<img width="10" />

<img src="https://skillicons.dev/icons?i=github" height="55" alt="GitHub" />
<img width="10" />

<img src="https://skillicons.dev/icons?i=vscode" height="55" alt="VSCode" />

</div>

---

## 🛡️ Cybersecurity Skills

* SOC Operations
* SIEM Monitoring
* Splunk
* Microsoft Sentinel
* Log Analysis
* Incident Response
* Threat Detection
* Threat Hunting
* Vulnerability Assessment
* Network Security
* Wireshark
* Phishing Analysis
* MITRE ATT&CK
* OSINT & Reconnaissance
* Security Monitoring
* Windows Event Logs
* Linux Security
* Nmap
* Kali Linux
* TCP/IP
* IDS/IPS Fundamentals
* Security Automation
* Blue Teaming

---

## 📜 Certifications & Learning

* CEH (Certified Ethical Hacker) – In Progress
* CCNA – In Progress
* LetsDefend SOC Labs
* Junior Cybersecurity Analyst Career Path
* Incident Handler
* Phishing Expert
* Networking Devices & Initial Configuration
* Accenture Data Analytics & Visualization Simulation

---

## 📌 Current Focus

* SOC Home Labs
* Threat Detection Labs
* Splunk Dashboards
* Blue Team Projects
* Security Automation
* Security Research & OSINT

---

## 🔥 Featured Projects

### 🔹 OpenDetect

OSINT-based username discovery tool for cybersecurity investigations and digital footprint analysis.

### 🔹 ScanFlow – Nmap Automation

Python-based Nmap automation framework for reconnaissance and network security workflows.

### 🔹 ScriptHound

Utility to search and organize Nmap scripts efficiently for penetration testing and security research.

### 🔹 Awesome Hacker Engines

Professional multi-engine OSINT and cybersecurity search tool.

---

## 🌐 Connect With Me

<div align="left">

<a href="https://linkedin.com/in/harshkatiyar5556" target="_blank">
<img src="https://img.shields.io/badge/LinkedIn-0077B5?style=for-the-badge&logo=linkedin&logoColor=white" />
</a>

<a href="https://medium.com/@harshkatiyar5556" target="_blank">
<img src="https://img.shields.io/badge/Medium-12100E?style=for-the-badge&logo=medium&logoColor=white" />
</a>

<a href="https://www.instagram.com/_kush__katiyar_/" target="_blank">
<img src="https://img.shields.io/badge/Instagram-E4405F?style=for-the-badge&logo=instagram&logoColor=white" />
</a>

</div>

---

<div align="center">

### 💻 Building • Learning • Defending

</div>
